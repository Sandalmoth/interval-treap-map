...
proc incl*[S, T](
  ...
  ## Insert a value ``val`` at a given interval ``key``
  ## returns a pointer that can be used with ``excl`` to remove the element
...
proc excl*[S, T](im: var IntervalMap[S, T], node: ptr Node[S, T]) =
  ## Remove node in the map by using a pointer to the element
